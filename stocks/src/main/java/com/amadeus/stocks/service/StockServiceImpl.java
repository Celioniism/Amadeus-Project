...
public class StockServiceImpl implements StockService {
	...
	private List<StockHistory> closingPrice(List<StockHistory> stocks) { // finds the most relevant price information from yesterday (closing price) 
	...
	private void Randomize() { // changes stock information recursively
		// I believe this could be refined with a base case and tuning to more accurately represent the stock market
		// and prevent eventual stack overflow errors due to recursive stack allocation issues that I may not have noticed.
		...
		if (now.isAfter(daystart) && now.isBefore(dayend)) {
			...
			for (StocksClass Stock : fluctuators) {
				...
				} else if (random > 0 && Stock.getPrice() < 50) {                 // list of random price changes to weigh out large spikes
			...
			System.out.println("time to complete: " + (end - start));  //just some interval checking to see if somehow my system is too clunky
			// I fixed stock history to be easier for the program to sort through (only 2 intervals are recorded per day open and close, with a more)
			//normalized database I could definitely store larger amounts of history per day.

			try {
				Thread.sleep(10000); //sleeping the current thread 
	...
	private List<StocksClass> calculate(List<StockHistory> hist) {
		//calculates change and percent change between stocks and retrieves them as a complete list.
		//
